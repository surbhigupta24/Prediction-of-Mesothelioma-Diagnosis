...
X = dataset.iloc[:,[6,17,8,0,2,8,32,25,18,23,1,26,27,22]].values  
...
clf4 = RandomForestClassifier(n_estimators = 100, random_state = 42, criterion="gini", max_depth=None, 
...
for i in alpha:
    ...
    sclf = StackingClassifier(classifiers=[sig_clf1, sig_clf2, sig_clf4], meta_classifier=lr, use_probas=True)
    sclf.fit(X_train, y_train)
    y_pred = sclf.predict(X_test)
    print("Stacking Classifer : for the value of alpha: %f Log Loss: %0.3f" % (i, log_loss(y_pred, sclf.predict_proba(X_test))))
    log_error =log_loss(y_pred, sclf.predict_proba(X_test))
    if best_alpha > log_error:
        best_alpha = log_error

lr = LogisticRegression(C=0.065)
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        classifier = StackingClassifier(classifiers=[sig_clf1, sig_clf4], meta_classifier=lr, 
                                        use_probas=True)
        classifier.fit(X_train, y_train)
        predicted_y.extend(classifier.predict(X_test))
        expected_y.extend(y_test)
